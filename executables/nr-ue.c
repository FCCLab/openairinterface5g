...
typedef enum { pss = 0, pbch = 1, si = 2, psbch = 3 } sync_mode_t;
...
static void UE_synch(void *arg) {
  ...
  if (UE->sl_mode == 2 && !get_softmodem_params()->sync_ref)
    sync_mode = psbch;
  //int CC_id = UE->CC_id;
  ...
  switch (sync_mode) {
    ...
    */
    ...
    case psbch:
      LOG_I(NR_PHY, "[UE thread Synch] Running Sidelink Initial Synch \n");
      NR_DL_FRAME_PARMS *fp = &UE->SL_UE_PHY_PARAMS.sl_frame_params;
      uint64_t sl_carrier = fp->sl_CarrierFreq;
      ret = sl_nr_slss_search(UE, &syncD->proc, SL_NR_SSB_REPETITION_IN_FRAMES);
      if (ret.cell_detected) {
        syncD->rx_offset = ret.rx_offset;
        freq_offset = UE->common_vars.freq_offset; // frequency offset computed with pss in initial sync
        hw_slot_offset = ((ret.rx_offset << 1) / fp->samples_per_subframe * fp->slots_per_subframe)
                         + round((float)((ret.rx_offset << 1) % fp->samples_per_subframe) / fp->samples_per_slot0);

        // rerun with new cell parameters and frequency-offset
        // todo: the freq_offset computed on DL shall be scaled before being applied to UL
        nr_rf_card_config_freq(&openair0_cfg[UE->rf_map.card], sl_carrier, sl_carrier, freq_offset);

        LOG_I(NR_PHY,
              "SL Got synch: hw_slot_offset %d, carrier off %d Hz, rxgain %f (SL-RX %f Hz, SL-TX %f Hz)\n",
              hw_slot_offset,
              freq_offset,
              openair0_cfg[UE->rf_map.card].rx_gain[0],
              openair0_cfg[UE->rf_map.card].rx_freq[0],
...
static int UE_dl_preprocessing(PHY_VARS_NR_UE *UE, const UE_nr_rxtx_proc_t *proc, int *tx_wait_for_dlsch, nr_phy_data_t *phy_data)
{
  ...
  if (proc->rx_slot_type == NR_DOWNLINK_SLOT || proc->rx_slot_type == NR_MIXED_SLOT) {
    ...
    if (UE->sl_mode == 2 && proc->rx_slot_type == NR_SIDELINK_SLOT) {
      if (UE->if_inst != NULL && UE->if_inst->sl_indication != NULL) {
...
void dummyWrite(PHY_VARS_NR_UE *UE,openair0_timestamp timestamp, int writeBlockSize) {
  ...
  int16_t dummy_tx_data[fp->nb_antennas_tx]
                       [2 * writeBlockSize]; // 2 because the function we call use pairs of int16_t implicitly as complex numbers

...
# SCENARIO = [datetime(2024, 8, 19, 16, 4, 30, tzinfo=local_tz).astimezone(timezone.utc).isoformat(),
#               datetime(2024, 8, 19, 16, 6, 0, tzinfo=local_tz).astimezone(timezone.utc).isoformat()
#             ]

SCENARIO = [datetime(2024, 8, 19, 19, 30, 45, tzinfo=local_tz).astimezone(timezone.utc).isoformat(),
              datetime(2024, 8, 19, 19, 33, 0, tzinfo=local_tz).astimezone(timezone.utc).isoformat()
            ]
...
total_cpu_usage = average_cu_up_1 + average_upf_slice_1 + average_cu_up_4 + average_upf_slice_4
...
plt.tight_layout()
plt.savefig('CPU_Usage_and_Throughput_without_S5.pdf', format='pdf', bbox_inches='tight')

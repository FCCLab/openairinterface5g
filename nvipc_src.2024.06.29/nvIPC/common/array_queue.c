    ...
    while(1)
    {
        ...
        else if(tail.node.next == node_tail.node.next && tail.node.counter == node_tail.node.counter)
        {
            if(tail.node.next == value)
            {
                ...
                NVLOGE_NO(TAG, AERIAL_NVIPC_API_EVENT, "%s: enqueue error link self: tail:%d-%u node_tail:%d-%u value=%d", priv_data->name, tail.node.next, tail.node.counter, node_tail.node.next, node_tail.node.counter, value);
            }
        ...
        {
            NVLOGE_NO(TAG, AERIAL_NVIPC_API_EVENT, "%s: enqueue error: tail:%d-%u node_tail:%d-%u value=%d", priv_data->name, tail.node.next, tail.node.counter, node_tail.node.next, node_tail.node.counter, value);
        }
    }
...
static int array_queue_dequeue(array_queue_t* queue)
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            NVLOGE_NO(TAG, AERIAL_NVIPC_API_EVENT, "%s: dequeue error: tail:%d-%u head:%d-%u node_head:%d-%u ", priv_data->name, tail.node.next, tail.node.counter, head.node.next, head.node.counter, node_head.node.next, node_head.node.counter);
        }
    }

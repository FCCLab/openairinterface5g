...
NR_BCCH_DL_SCH_Message_t *get_SIB1_NR(const NR_ServingCellConfigCommon_t *scc, const f1ap_plmn_t *plmn, uint64_t cellID, int tac, const gNB_MAC_INST *mac)
{
  ...
  *ZoneEight = 0;
  ...
  sib1->ue_TimersAndConstants->t300 = get_NR_UE_TimersAndConstants_t300(&mac->timer_config);
  sib1->ue_TimersAndConstants->t301 = get_NR_UE_TimersAndConstants_t301(&mac->timer_config);
  sib1->ue_TimersAndConstants->t310 = get_NR_UE_TimersAndConstants_t310(&mac->timer_config);
  sib1->ue_TimersAndConstants->n310 = get_NR_UE_TimersAndConstants_n310(&mac->timer_config);
  sib1->ue_TimersAndConstants->t311 = get_NR_UE_TimersAndConstants_t311(&mac->timer_config);
  sib1->ue_TimersAndConstants->n311 = get_NR_UE_TimersAndConstants_n311(&mac->timer_config);
  sib1->ue_TimersAndConstants->t319 = get_NR_UE_TimersAndConstants_t319(&mac->timer_config);
                                                  ...
                                                  const gNB_MAC_INST *mac)
{
  ...
  cellGroupConfig->mac_CellGroupConfig = configure_mac_cellgroup(&mac->timer_config);

  cellGroupConfig->physicalCellGroupConfig = configure_phy_cellgroup();

  cellGroupConfig->spCellConfig = get_initial_SpCellConfig(uid, scc, servingcellconfigdedicated, &mac->radio_config);
                                                     ...
                                                     int uid)
{
  ...
  secondaryCellGroup->mac_CellGroupConfig = configure_mac_cellgroup(&mac->timer_config);

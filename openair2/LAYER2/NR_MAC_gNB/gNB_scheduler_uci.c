...
static bool get_measured_rsrp(uint8_t index, int *rsrp)
{
  ...
  if (index <= 15){
    LOG_I(NR_MAC, "get_measured_rsrp index: %d\n", index);
    return false;
  }
  if (index >= 114){
    LOG_I(NR_MAC, "get_measured_rsrp index: %d\n", index);
...
static void tci_handling(NR_UE_info_t *UE, frame_t frame, slot_t slot)
{
  ...
  */
  ...
  if (!valid) {
    LOG_E(NR_MAC, "UE %04x: reported RSRP index %d invalid - tci_handling\n", UE->rnti, rsrp_index);
    return;
  }
                                 ...
                                 NR_CSI_ReportConfig__reportQuantity_PR reportQuantity_type)
{
  ...
  *cumul_bits += 7;
  ...
  bool valid = get_measured_rsrp(rsrp_index, &strongest_ssb_rsrp);
  if (!valid) {
    LOG_E(NR_MAC, "UE %04x: reported RSRP index %d invalid - evaluate_rsrp_report\n", UE->rnti, rsrp_index);
    return;
  }

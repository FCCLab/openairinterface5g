...
void nr_ue_create_ip_if(const char *ifname, int ue_id, int pdu_session_id, bool ipv6, const uint8_t addr[IPV4V6_ADDR_LEN])
{
  ...
  snprintf(ifname_suffix, sizeof(ifname_suffix), "p%d", pdu_session_id);
  int default_pdu = softmodem_nas_info.default_pdu_session_id;
  nas_config_interface_name(ue_id,
                            ifname,
                            (pdu_session_id != default_pdu) ? ifname_suffix : NULL,
                            ifname_full,
                            sizeof(ifname_full));
  if (doesInterfaceExist(ifname_full)) {
...
static void request_pdusession(const nr_nas_pdp_context_t *pdp_cxt, int instance)
{
  ...
  if (msg_p != NULL) {
	...
	case FGS_PDU_SESSION_ESTABLISHMENT_REJ:
      ...
      case NAS_PDU_SESSION_REL: {
        ...
        nr_sdap_delete_entity(instance, pdu_rel->pdu_session_id, true);
        break;
      }
